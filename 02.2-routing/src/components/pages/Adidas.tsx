...
export const Adidas = () => {
    const adidasArr = data.filter(el => el.brand === 'adidas')
    return (
        <div>
            <h2 style={{textAlign: 'center'}}> ADIDAS</h2>
            {adidasArr.map((el, idx) => {
                return <NavLink key={el.id} to={`/${el.brand}/${el.id}`}>
                    <img src={el.picture} alt={el.model} style={{width: '200px'}}/>
                </NavLink>
            })}
            <p>
                What is Lorem Ipsum?
